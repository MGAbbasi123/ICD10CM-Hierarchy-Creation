...
def load_icd_hierarchy():
    ...
    # Initialize parent stack and ancestor columns for each level
    parent_stack = {}
    ...
    # Reorder columns: main columns + paired level columns + top-level code
    level_columns = []
    for i in range(1, max_indent + 1):
        level_columns.extend([f'LEVEL_{i}_PARENT', f'LEVEL_{i}_DESC'])

    base_columns = ['ORDER', 'ICD_CODE', 'DESCRIPTION', 'INDENT_LEVEL']
    final_columns = base_columns + level_columns + ['TOP_LEVEL_CODE']

    df = df[final_columns]

    # Preview first 10 rows in terminal
    ...
    print(f"\n✅ Hierarchy rebuilt dynamically up to Level {max_indent} with paired descriptions.")
